#-------------------------------------------------------------------------------
# Modified by `https://github.com/t-wilkinson`
#-------------------------------------------------------------------------------
# Done:
#   Added functionality for optional prepended string
#-------------------------------------------------------------------------------
# Todo:
#   Links with anchor work as follows:
#       [text](href#anchor) → #[anchor](…)
#-------------------------------------------------------------------------------
# Note:
#   This package does not support files without extensions and i'm too lazy to
#   fix that
#-------------------------------------------------------------------------------
...
# LINE = PREPEND + "[name](href.md#anchor)"
...
_logger = FakeLogger(active=False)
...
def open_link(target, current_file, open_in_vim_extensions=set()):
    ...
    if target.startswith('#'):
        _logger.info('jump to anchor %s', target)
...
def parse_path(path):
    ...
    # return ParsedPath(path + ext, anchor=path.rsplit('#', 1)[1])
...
def select_from_start_of_link(line, col):
    """Return the start of the link string and the new cursor
    """
    pattern = re.compile(r'\[.*?\]\(.*?\)')
    matches = pattern.finditer(line, 0)
    start = None

    # Set 'start' to start of next link (or the last one)
    for m in matches:
        start = m.start()
        if col <= m.start() or m.start() < col < m.end():
            break
